    ...
    class ViewHolder(
        ...
        fun bindItem(items: Event, listener: (Event) -> Unit) {
            ...
            doAsync {
                ...
                uiThread {
                    teamAway.teams[0].badge.let {  Picasso.get().load(it).into(containerView.AwayLogo) }
                    teamHome.teams[0].badge.let {  Picasso.get().load(it).into(containerView.homeTeamLogo) }
                }
            }
        }
    }
